...
import { formatDateTime, parseTaskDate } from "../../utils/dateUtils";
...
function EditTaskModal({ task, lang, error, onClose, onEditTask }) {
  const [taskData, setTaskData] = useState({
    title: task.title,
    description: task.description,
    deadline: task.deadline ? parseTaskDate(task.deadline) : null,
    ...
    <div className={`newtask-modal-overlay ${isModalOpen ? "modal-open" : ""}`}>
      <div className="newtask-modal-content">
        ...
        <h2 className="modal-title">
          {lang === "eng" ? "Edit Task" : "Изменить задачу"}
        </h2>
              ...
              {["low", "medium", "high"].map((level, index) => (
                <label key={index + 1} className="priority-option">
                  <input
                    ...
                    name="priority"
                    value={index + 1}
                    checked={Number(taskData.priority) === index + 1}
                    onChange={handleChange}
                  />
